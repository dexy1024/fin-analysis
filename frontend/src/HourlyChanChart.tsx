...
export function HourlyChanChart({
  ...
  const lastPenIsUp = lastPen?.direction === 'up'
  const penDirLabel = lastPen
    ? lastPenIsUp
      ? '向上'
      : '向下'
    : '—'
  const penDirDisplay = lastPen ? (lastPenIsUp ? '一笔上' : '一笔下') : '—'
  const penDirColor = lastPen ? (lastPenIsUp ? '#22c55e' : '#ef4444') : undefined
  ...
  // 与当前 K 线同步：由 computeHourlyBuySellState 实时计算，不依赖后端雷达缓存
  const buyConditionChecklist = [
        ...
        <aside className="central-compare-aside" aria-label={`${periodLabel}现价与日线中枢对比`}>
          ...
          {indexKline.period === '15' ? (
            <>
              ...
              {lastPen && (
                <div className="central-compare-row" style={{ marginTop: '0.75rem' }}>
                  <span className="central-compare-label">当前笔（15min）</span>
                  <span
                    className="central-compare-ref"
                    style={{ color: penDirColor, fontWeight: 600 }}
                  >
                    {penDirDisplay}
                  </span>
                </div>
