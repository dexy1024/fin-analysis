...
**章节来源**
- [backend/main.py: 路由与生命周期:80-107](file://backend/main.py#L80-L107)
...
## 架构总览
后端通过 Gunicorn + Uvicorn Worker 承载 FastAPI 应用，前端通过 Vite 提供开发与预览服务。定时任务在后端进程中独立线程运行，负责数据同步与雷达计算。SSE 用于向前端推送雷达更新。
...
**图表来源**
- [restart_services.sh: 启动后端与前端:81-87](file://restart_services.sh#L81-L87)
...
**图表来源**
- [restart_services.sh: 端口清理与启动:56-87](file://restart_services.sh#L56-L87)
- [restart_services.sh: 健康检查:99-111](file://restart_services.sh#L99-L111)

**章节来源**
- [restart_services.sh: 完整实现:1-126](file://restart_services.sh#L1-L126)
...
**章节来源**
- [restart_services.sh: 代理与环境注入:26-30](file://restart_services.sh#L26-L30)
...
**章节来源**
- [restart_services.sh: 日志输出与命名:77-79](file://restart_services.sh#L77-L79)
- [backend/main.py: 日志配置:72-77](file://backend/main.py#L72-L77)
...
**章节来源**
- [restart_services.sh: 后端启动参数:82-83](file://restart_services.sh#L82-L83)
...
- [backend/main.py: SSE 客户端队列与清理:54-70](file://backend/main.py#L54-L70)
...
- [backend/main.py: 健康端点与摘要端点:171-186](file://backend/main.py#L171-L186)

## 结论
本部署配置文档提供了从环境准备、脚本使用、服务启动到监控与故障排查的完整指引。通过合理配置环境变量、规范使用部署脚本与数据更新脚本，并结合日志与健康检查，可稳定地在生产环境中运行本项目。
...
**章节来源**
- [backend/main.py: 路由定义:171-252](file://backend/main.py#L171-L252)
