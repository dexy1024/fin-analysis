...
- **市场状态集成简化**：将"三层数控系统"简化为"参考显示"，仅作参考展示，不再驱动个股交易决策
...
- **交易指令引擎**：**简化为"参考显示"模式**：三层风控（大盘、个股区间套、终极状态机）仅作参考展示，不再驱动交易决策。
...
- **风控层**：**简化为"参考显示"模式**：大盘风控、区间套风控与状态机决策仅作参考展示。
...
Scheduler->>Engine : 生成交易报告仅参考
Signals-->>Frontend : buy_sell_signals.json
Radar-->>Frontend : 雷达摘要
Engine-->>Frontend : 报告与状态仅参考
```
...
**更新** 交易指令引擎集成了更严格的MACD分析和验证逻辑，**但市场状态集成已简化为"参考显示"**

- **三层体系简化**：
  1) **全局大盘状态（上证指数000001）**：MARKET_DEAD/MARKET_DANGER/MARKET_SAFE
     - **仅作参考展示**：不再驱动个股交易决策
...
```mermaid
flowchart TD
E0["获取日线/60分钟数据"] --> E1["计算大盘风控状态仅参考"]
E1 --> E2{"市场状态"}
E2 --> |MARKET_DEAD| E3["仅作参考不强制清仓"]
E2 --> |MARKET_DANGER| E4["仅作参考不禁买"]
E2 --> |MARKET_SAFE| E5["个股区间套风控"]
E5 --> E6["计算60分钟条件"]
E6 --> E7{"是否满足买入条件"}
E7 --> |是| E8["BUY仅参考"]
E7 --> |否| E9["HOLD/IGNORE仅参考"]
```
...
本系统通过"K线与指标 → 信号检测 → 风控与执行 → 可视化"的完整链路，实现了高可靠性的买卖信号分析与自动化交易支持。经过MACD分析和验证程序的重构，系统的准确性和一致性得到了显著提升。后端模块化设计与前端可视化协同，确保了信号质量与用户体验。

**重要变更**：市场状态集成已简化为"参考显示"模式，三层风控体系中的大盘状态仅作参考展示，不再驱动个股交易决策。这一简化提升了系统的稳定性和可靠性，同时保持了信号分析的准确性。

建议持续优化参数与阈值，完善回测与评估体系，以提升信号成功率与稳定性。
...
**市场状态参考**：由于市场状态集成已简化为"参考显示"，回测时应重点关注信号本身的准确性和MACD分析的质量，而非依赖市场状态驱动的交易决策。