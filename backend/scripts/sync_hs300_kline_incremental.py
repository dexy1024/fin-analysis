...
def main() -> int:
    ...
    logging.info(
        "完成: 标的数=%d dry_run=%s periods=%s ok_60=%d fail_60=%d ok_15=%d fail_15=%d",
        summary.symbols,
        args.dry_run,
        periods,
        summary.ok_60,
    ...
    if args.dry_run:
        for r in summary.results[:20]:
            logging.info(
                "dry-run %s %s | start_60=%s | start_15=%s",
                r.code,
                r.name or "-",
                r.start_60,
                r.start_15,
    ...
    return 0 if summary.fail_60 == 0 and summary.fail_15 == 0 else 1
