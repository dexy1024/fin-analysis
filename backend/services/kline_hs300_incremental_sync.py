"""
沪深300（watchlist_hs300.json）60m/15m K 线：手动增量同步。

- 读取本地 data/kline_{60|15}_{code}.csv 中最后一根 K 的 date，作为拉网窗口起点（与缓存重叠一根，
  写入时由 _merge_kline_ohlcv_with_cache 按 date 去重 keep last）。
- 若文件不存在、为空、或命中「全日 15:00」伪缓存，则起点与 kline_scheduler 一致：60m=79 自然日、15m=25 自然日。
"""
...
Period = Literal["60", "15"]


def _cold_start_start(period: Period) -> str:
...
def last_bar_timestamp(symbol: str, period: Period) -> Optional[pd.Timestamp]:
...
def incremental_start_date(symbol: str, period: Period) -> str:
...
def _sync_symbol(
    ...
    *,
    ...
    s60 = incremental_start_date(code, "60")
    s15 = incremental_start_date(code, "15")
    res = SymbolSyncResult(code=code, name=name, start_60=s60, start_15=s15, dry_run=dry_run)
    if dry_run:
        return res
    for p in periods:
        start = incremental_start_date(code, p)
        try:
            n = sync_minute_kline_to_csv(code, p, start_date=start, end_date=None)
            if p == "60":
                res.rows_60 = n
                res.touched_60 = True
            else:
